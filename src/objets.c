...
void tresor_sans_issue(Labyrinthe *l){
	int i,j;
	for(i = 0; i < NB_LIG; i++){
		for(j = 0; j < NB_COL; j++){
...
void ajout_mob(Labyrinthe *l,int pourcentage){
	int i,j,random;
	for(i = 0; i < NB_LIG; i++){
		for(j = 0; j < NB_COL; j++){
			if(l->floor[i][j].type == ROOM){
...
void ajout_escalier(Labyrinthe *l){
	...
	for(i = 0; i < NB_LIG; i++){
		for(j = 0; j < NB_COL; j++){
			if(l->floor[i][j].type == ROOM && distance_de_Manhattan(i,j,NB_LIG/2,NB_COL/2) <= 15/*>=25*/)
